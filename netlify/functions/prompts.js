...
const FILLER_OUTCOME = {
  ...
  nose: {
    ...
    moderate: 'a clearly visible non-surgical nose refinement, roughly 1.5 mL of HA filler (liquid rhinoplasty): a noticeably straighter, smoother side profile, a higher but natural bridge, and improved tip projection and support that reads clearly in side-by-side comparison. Keep the result soft and appropriate for this face, keep the frontal nose width unchanged, do not narrow the nose, and do not create a high, narrow, surgical, or European-style nose',
    // M17 #2: enhanced was too subtle on conservative gpt-image-2 -- with the new
    // primary feature-composite (Approach B) taking ONLY the nose region, a barely
    // changed nose reads as untreated. Push magnitude (higher bridge, straighter
    // profile, more tip projection) and demand visibility at BOTH frontal and
    // oblique, while keeping every anti-European / no-narrow / keep-width guard.
    enhanced: 'a strong, clearly and obviously visible non-surgical nose refinement, about 2 mL of HA filler (liquid rhinoplasty), as an experienced injector would place it: clearly build up the bridge so the dorsum sits distinctly higher and the side profile reads markedly straighter and smoother, with a clean continuous dorsal line from between the brows to the tip, add clear radix support, and give the tip visibly more projection and support so it looks lifted and better defined. The improvement must be immediately obvious in side-by-side comparison at BOTH the frontal and the three-quarter view, not a subtle change; if uncertain, prefer a more visible result. Stay believable for HA filler and never surgical. Do NOT narrow the nose and keep the frontal nose width and nostril width exactly as in the original, keep the result soft and appropriate for this face, and do not create a high, thin, pinched, over-projected, surgical, or European-style nose'
  },
  nasolabial_folds: {
